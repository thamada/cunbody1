//Time-stamp: <2008-05-01 18:06:11 hamada>
...
namespace cunbody_kernel_type00{
...
#define BID (blockIdx.x)
  ...
  kernel(float4* g_xj,
	 ...
	 int nj)
  {
    int n_jblock = (nj+NJ_SHMEM-1)/NJ_SHMEM;        // NJ_SHMEM is defined in cunbody_api.h
    ...
    for(int jloop = 0; jloop<n_jblock; jloop++){ //------------------ J-BLOCK Loop
      int j_start = NJ_SHMEM * jloop;
      __syncthreads();
      s_xj[TID] = g_xj[j_start+TID];    // ** Technic ** 
      __syncthreads();

#ifdef AC
#undef AC
#endif
#define AC(j,a,b)  inter(s_xj[(j)], xi, (a), (b));  
